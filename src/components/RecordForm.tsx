...
export function RecordForm({ initialDate, exercises, submitLabel = '追加する', onSubmit, onSubmitted }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onSubmit({ date, exercise: selectedExercise, reps: value });
      // 日付以外の項目を入力前の状態に戻し、続けて登録できるようにする
  ...
  if (exercises.length === 0) {
    return (
      <p className="muted">先に「トレーニング管理」からトレーニング内容を登録してください。</p>
    );
  }
